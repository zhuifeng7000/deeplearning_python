# -*- coding: utf-8 -*-
"""
Created on Wed Jun 20 10:26:15 2018
...
def test():
    dataM,label_arr=load_data_set('horseColicTraining2.txt')
    print('dataM.shape=',dataM.shape)
    print(dataM)
    print('label_arr lent=',len(label_arr))
    #构建并且训练
    weak_class_arr, agg_class_est = ada_boost_train_ds(dataM, label_arr, 40)
...
if __name__ == '__main__':
    ...
    #处理数据，以平均数补足空缺数据,测试了一下，好像没有作用
    dataM=dataM + np.multiply((dataM[:,:]==0),np.mean(dataM,axis=0))
    #构建并且训练
    weak_class_arr, agg_class_est = ada_boost_train_ds(dataM, label_arr, 40)
    print(weak_class_arr, '\n-----\n', agg_class_est.T)
    #绘图
    ...
    #测试样本集
    data_arr_test, label_arr_test = load_data_set("horseColicTest2.txt")
    #data_arr_test=data_arr_test + np.multiply((data_arr_test[:,:]==0),np.mean(data_arr_test,axis=0))
