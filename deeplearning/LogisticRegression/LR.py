...
def propagate(w, b, X, Y):
    ...
    #J = (1/m) * (-1)*np.sum(Y*np.log(A)+(1-Y)*np.log(1-A))
...
#    print ("predictions = " + str(predict(w, b, X)))
    ...
    mimage = scipy.misc.imresize(image,size=(64,64)).reshape(1,64*64*3).T
    print("mimage shape=",mimage.shape)
    my_predicted_image = predict(d['w'],d['b'],mimage)
    plt.imshow(image)
